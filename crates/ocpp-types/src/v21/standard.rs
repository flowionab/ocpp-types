...
pub enum ComponentName {
    /// Responsible for configuration relating to DER capabilities that the EVSE of the Charging Station can emulate by using ISO 15118-20 ChargeLoop messages to control the inverter in the EV. The component is located at the EVSE level, since it represents the DER capabilities of the EVSE.
    ACDERCtrlr,
    ...
    AuthCtrlr,
    /// Responsible for configuration relating to Battery swapping.
    BatterySwapCtrlr,
    ...
    CustomizationCtrlr,
    /// Responsible for configuration relating to DER capabilities of the DC inverter of the EVSE in the Charging Station. The component is located at the EVSE level, since it represents the DER capabilities, also referred to as nameplate information, of the EVSE.
    DCDERCtrlr,
    ...
    MonitoringCtrlr,
    /// Logical Component responsible for configuration relating to payment terminals.
    PaymentCtrlr,
    ...
    TxCtrlr,
    /// Responsible for configuration relating to V2X charging/discharging. This component exists on the EVSE tier hierarchy.
    V2XChargingCtrlr,
    /// Responsible for configuration of a dynamic QR code for ad hoc payments.
    WebPaymentsCtrlr,
    ...
    AreaVentilation,
    /// BatteryCartridge represents the battery cartridge that is currently inserted into the EVSE of a battery swap station
    BatteryCartridge,
...
impl ComponentName {
    /// Every value this version's specification defines (84), in spec order.
    ///
    ...
    pub const ALL: &'static [Self] = &[
        ...
        Self::TxCtrlr,
        Self::V2XChargingCtrlr,
        Self::WebPaymentsCtrlr,
        Self::AccessBarrier,
        Self::AcDcConverter,
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            Self::PaymentCtrlr => "PaymentCtrlr",
            ...
            Self::V2XChargingCtrlr => "V2XChargingCtrlr",
            Self::WebPaymentsCtrlr => "WebPaymentsCtrlr",
            ...
            Self::BatteryCartridge => "BatteryCartridge",
    ...
    pub fn from_wire(value: &str) -> Option<Self> {
        match value {
            ...
            "PaymentCtrlr" => Some(Self::PaymentCtrlr),
            ...
            "V2XChargingCtrlr" => Some(Self::V2XChargingCtrlr),
            "WebPaymentsCtrlr" => Some(Self::WebPaymentsCtrlr),
            ...
            "BatteryCartridge" => Some(Self::BatteryCartridge),
...
pub enum VariableName {
    ...
    /// For TariffCostCtrlr: Specifies the maximum number of _prices_ elements that the Charging Station supports in each _energy_, _chargingTime, _idleTime_ and _fixedFee_ of a TariffType.
    MaxElements,
    ...
    /// Defines which _setpoint_ shall be used when a `ChargingStationExternalConstraints` profile
    SetpointPriority,
    ...
    SupplyPhases,
    /// This configuration variable lists the additional charging profile purposes, that have been introduced in OCPP 2.1, that are supported by the Charging Station.
    ...
    /// For DisplayMessageCtrlr: List of message formats supported by this Charging Station.
    SupportedFormats,
    /// The subset of the list of supported IdTokenTypes as defined in Appendix 7.
    ...
    SupportedLimits,
    /// Lists the V2X operation modes that are supported by the Charging Station.
    SupportedOperationModes,
    /// For DisplayMessageCtrlr: List of the priorities supported by this Charging Station.
    SupportedPriorities,
    ...
    SupportedProviders,
    /// For DisplayMessageCtrlr: List of the states during which to display a message supported by this Charging Station.
    SupportedStates,
    /// When this variable has value True, then the Charging Station supports charging profiles of type `Dynamic`.
    SupportsDynamicProfiles,
    /// When reported as true the Charging Station supports the _evseSleep_ flag in a ChargingSchedulePeriod, which requests the EVSE electronics to go to sleep during _operationMode_ = 'Idle'.
    SupportsEvseSleep,
    ...
    /// When this variable has value True, then the Charging Station supports the field _limitAtSoC_ in ChargingSchedul, which will cap the limit or setpoint in the ChargingSchedulePeriodType by the value of _limitAtSoC.limit._
    SupportsLimitAtSoC,
    /// When this variable has value True, then the Charging Station supports the fields _maxOfflineDuration_ and _invalidAfterOfflineDuration_ in ChargingProfile.
    SupportsMaxOfflineDuration,
    /// When this variable has value True, then the Charging Station supports the field _randomizedDelay_ in ChargingSchedule, which will delay the start of each charging schedule period by a random number between 0 and _randomizedDelay_.
    SupportsRandomizedDelay,
    /// When this variable has value True, then the Charging Station supports the field _useLocalTime_ in ChargingSchedule.
    ...
    Suspension,
    /// For BatterySwapCtrlr: The state of charge that a battery must have in order to be eligible for swapping.
    TargetSoc,
    ...
    /// Interval between sampling of metering (or other) data, intended to be transmitted via TransactionEventRequest(Updated) messages for location = `Upstream` only.
    UpstreamInterval,
    /// Sampled measurands to be included in the _meterValues_ element of every TransactionEventRequest(Updated) for location = `Upstream` only.
    UpstreamMeasurands,
    ...
    VoltageImbalance,
    /// This variable represents the current working mode of the battery in BatteryCartridge component.
    WorkingMode,
    /// Listed in the specification's device model table but not in its dedicated name table.
    VendorName,
    /// Listed in the specification's device model table but not in its dedicated name table.
    ...
    InverterHwVersion,
    /// Listed in the specification's device model table but not in its dedicated name table.
    IslandingDetectionMethod,
    /// Listed in the specification's device model table but not in its dedicated name table.
    IslandingDetectionTripTime,
    /// Listed in the specification's device model table but not in its dedicated name table.
    ReactiveSusceptance,
    /// Listed in the specification's device model table but not in its dedicated name table.
    TargetSoC,
    ...
    AuthorizationAmount,
    /// Listed in the specification's device model table but not in its dedicated name table.
    IncrementalAuthorizationAmount,
    /// Listed in the specification's device model table but not in its dedicated name table.
    IncrementalAuthorizationThreshold,
    /// Listed in the specification's device model table but not in its dedicated name table.
    PaymentDetails,
    /// Listed in the specification's device model table but not in its dedicated name table.
    SettlementByCSMS,
...
impl VariableName {
    /// Every value this version's specification defines (328), in spec order.
    ///
    /// Does not include [`Self::Other`], which is unbounded in the
    /// values it can hold.
    pub const ALL: &'static [Self] = &[
        ...
        Self::LocalFrequencyUpdateThreshold,
        ...
        Self::Storage,
        Self::SupplyPhases,
        Self::SupportedAdditionalPurposes,
        Self::SupportedEnergyTransferModes,
        Self::SupportedFormats,
        Self::SupportedIdTokenTypes,
        Self::SupportedLimits,
        Self::SupportedOperationModes,
        Self::SupportedPriorities,
        Self::SupportedProviders,
        Self::SupportedStates,
        Self::SupportsDynamicProfiles,
        Self::SupportsEvseSleep,
        Self::SupportsExpiryDateTime,
        Self::SupportsLimitAtSoC,
        Self::SupportsMaxOfflineDuration,
        Self::SupportsRandomizedDelay,
        Self::SupportsUseLocalTime,
        Self::Suspending,
        Self::Suspension,
        Self::TargetSoc,
        ...
        Self::UpstreamInterval,
        Self::UpstreamMeasurands,
        ...
        Self::MaxVarNeg,
        Self::MaxChargeRateW,
        Self::MaxChargeRateVA,
        Self::VNom,
        Self::MaxV,
        Self::MinV,
        Self::ModesSupported,
        Self::InverterManufacturer,
        Self::InverterModel,
        Self::InverterSerialNumber,
        Self::InverterSwVersion,
        Self::InverterHwVersion,
        Self::IslandingDetectionMethod,
        Self::IslandingDetectionTripTime,
        Self::ReactiveSusceptance,
        Self::TargetSoC,
        Self::OcppCsmsUrl,
        Self::OcppInterface,
        Self::OcppTransport,
        ...
        Self::AuthorizeDirectPayment,
        Self::AuthorizationAmount,
        Self::IncrementalAuthorizationAmount,
        Self::IncrementalAuthorizationThreshold,
        Self::PaymentDetails,
        Self::SettlementByCSMS,
        Self::ReceiptServerUrl,
        Self::ReceiptByCSMS,
        Self::Merchant,
        Self::TerminalID,
        Self::PaymentServiceProvider,
        Self::Connected,
        Self::URLTemplate,
        Self::URLParameters,
        Self::TOTPVersion,
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            Self::HandleFailedTariff => "HandleFailedTariff",
            ...
            Self::LocalFrequencyUpdateThreshold => "LocalFrequencyUpdateThreshold",
            ...
            Self::MaxPeriodicEventStreams => "MaxPeriodicEventStreams",
            ...
            Self::SetpointPriority => "SetpointPriority",
            ...
            Self::SupportedAdditionalPurposes => "SupportedAdditionalPurposes",
            Self::SupportedEnergyTransferModes => "SupportedEnergyTransferModes",
            Self::SupportedFormats => "SupportedFormats",
            Self::SupportedIdTokenTypes => "SupportedIdTokenTypes",
            Self::SupportedLimits => "SupportedLimits",
            Self::SupportedOperationModes => "SupportedOperationModes",
            Self::SupportedPriorities => "SupportedPriorities",
            Self::SupportedProviders => "SupportedProviders",
            Self::SupportedStates => "SupportedStates",
            Self::SupportsDynamicProfiles => "SupportsDynamicProfiles",
            Self::SupportsEvseSleep => "SupportsEvseSleep",
            Self::SupportsExpiryDateTime => "SupportsExpiryDateTime",
            Self::SupportsLimitAtSoC => "SupportsLimitAtSoC",
            Self::SupportsMaxOfflineDuration => "SupportsMaxOfflineDuration",
            Self::SupportsRandomizedDelay => "SupportsRandomizedDelay",
            Self::SupportsUseLocalTime => "SupportsUseLocalTime",
            ...
            Self::UpstreamInterval => "UpstreamInterval",
            Self::UpstreamMeasurands => "UpstreamMeasurands",
            Self::V2GCertificateInstallationEnabled => {
                "V2GCertificateInstallationEnabled"
            }
            ...
            Self::MaxW => "MaxW",
            Self::OverExcitedW => "OverExcitedW",
            Self::OverExcitedPF => "OverExcitedPF",
            Self::UnderExcitedW => "UnderExcitedW",
            Self::UnderExcitedPF => "UnderExcitedPF",
            Self::MaxVA => "MaxVA",
            Self::MaxVar => "MaxVar",
            Self::MaxVarNeg => "MaxVarNeg",
            Self::MaxChargeRateW => "MaxChargeRateW",
            Self::MaxChargeRateVA => "MaxChargeRateVA",
            Self::VNom => "VNom",
            Self::MaxV => "MaxV",
            Self::MinV => "MinV",
            Self::ModesSupported => "ModesSupported",
            Self::InverterManufacturer => "InverterManufacturer",
            Self::InverterModel => "InverterModel",
            Self::InverterSerialNumber => "InverterSerialNumber",
            Self::InverterSwVersion => "InverterSwVersion",
            Self::InverterHwVersion => "InverterHwVersion",
            Self::IslandingDetectionMethod => "IslandingDetectionMethod",
            Self::IslandingDetectionTripTime => "IslandingDetectionTripTime",
            Self::ReactiveSusceptance => "ReactiveSusceptance",
            Self::TargetSoC => "TargetSoC",
            ...
            Self::AuthorizeDirectPayment => "AuthorizeDirectPayment",
            Self::AuthorizationAmount => "AuthorizationAmount",
            Self::IncrementalAuthorizationAmount => "IncrementalAuthorizationAmount",
            Self::IncrementalAuthorizationThreshold => {
                "IncrementalAuthorizationThreshold"
            }
            Self::PaymentDetails => "PaymentDetails",
            Self::SettlementByCSMS => "SettlementByCSMS",
            Self::ReceiptServerUrl => "ReceiptServerUrl",
            Self::ReceiptByCSMS => "ReceiptByCSMS",
    ...
    pub fn from_wire(value: &str) -> Option<Self> {
        match value {
            ...
            "DischargePower" => Some(Self::DischargePower),
            ...
            "HandleFailedTariff" => Some(Self::HandleFailedTariff),
            ...
            "LocalFrequencyUpdateThreshold" => Some(Self::LocalFrequencyUpdateThreshold),
            ...
            "SetpointPriority" => Some(Self::SetpointPriority),
            ...
            "SupportedAdditionalPurposes" => Some(Self::SupportedAdditionalPurposes),
            "SupportedEnergyTransferModes" => Some(Self::SupportedEnergyTransferModes),
            "SupportedFormats" => Some(Self::SupportedFormats),
            "SupportedIdTokenTypes" => Some(Self::SupportedIdTokenTypes),
            "SupportedLimits" => Some(Self::SupportedLimits),
            "SupportedOperationModes" => Some(Self::SupportedOperationModes),
            "SupportedPriorities" => Some(Self::SupportedPriorities),
            "SupportedProviders" => Some(Self::SupportedProviders),
            "SupportedStates" => Some(Self::SupportedStates),
            "SupportsDynamicProfiles" => Some(Self::SupportsDynamicProfiles),
            "SupportsEvseSleep" => Some(Self::SupportsEvseSleep),
            "SupportsExpiryDateTime" => Some(Self::SupportsExpiryDateTime),
            "SupportsLimitAtSoC" => Some(Self::SupportsLimitAtSoC),
            "SupportsMaxOfflineDuration" => Some(Self::SupportsMaxOfflineDuration),
            "SupportsRandomizedDelay" => Some(Self::SupportsRandomizedDelay),
            "SupportsUseLocalTime" => Some(Self::SupportsUseLocalTime),
            "Suspending" => Some(Self::Suspending),
            "Suspension" => Some(Self::Suspension),
            "TargetSoc" => Some(Self::TargetSoc),
            ...
            "UpstreamInterval" => Some(Self::UpstreamInterval),
            "UpstreamMeasurands" => Some(Self::UpstreamMeasurands),
            "V2GCertificateInstallationEnabled" => {
            ...
            "WorkingMode" => Some(Self::WorkingMode),
            ...
            "MaxW" => Some(Self::MaxW),
            "OverExcitedW" => Some(Self::OverExcitedW),
            "OverExcitedPF" => Some(Self::OverExcitedPF),
            "UnderExcitedW" => Some(Self::UnderExcitedW),
            "UnderExcitedPF" => Some(Self::UnderExcitedPF),
            "MaxVA" => Some(Self::MaxVA),
            "MaxVar" => Some(Self::MaxVar),
            "MaxVarNeg" => Some(Self::MaxVarNeg),
            "MaxChargeRateW" => Some(Self::MaxChargeRateW),
            "MaxChargeRateVA" => Some(Self::MaxChargeRateVA),
            "VNom" => Some(Self::VNom),
            "MaxV" => Some(Self::MaxV),
            "MinV" => Some(Self::MinV),
            "ModesSupported" => Some(Self::ModesSupported),
            "InverterManufacturer" => Some(Self::InverterManufacturer),
            "InverterModel" => Some(Self::InverterModel),
            "InverterSerialNumber" => Some(Self::InverterSerialNumber),
            "InverterSwVersion" => Some(Self::InverterSwVersion),
            "InverterHwVersion" => Some(Self::InverterHwVersion),
            "IslandingDetectionMethod" => Some(Self::IslandingDetectionMethod),
            "IslandingDetectionTripTime" => Some(Self::IslandingDetectionTripTime),
            "ReactiveSusceptance" => Some(Self::ReactiveSusceptance),
            "TargetSoC" => Some(Self::TargetSoC),
            ...
            "AuthorizeDirectPayment" => Some(Self::AuthorizeDirectPayment),
            "AuthorizationAmount" => Some(Self::AuthorizationAmount),
            "IncrementalAuthorizationAmount" => {
                Some(Self::IncrementalAuthorizationAmount)
            }
            "IncrementalAuthorizationThreshold" => {
                Some(Self::IncrementalAuthorizationThreshold)
            }
            "PaymentDetails" => Some(Self::PaymentDetails),
            "SettlementByCSMS" => Some(Self::SettlementByCSMS),
            "ReceiptServerUrl" => Some(Self::ReceiptServerUrl),
            "ReceiptByCSMS" => Some(Self::ReceiptByCSMS),
            "Merchant" => Some(Self::Merchant),
            "TerminalID" => Some(Self::TerminalID),
            "PaymentServiceProvider" => Some(Self::PaymentServiceProvider),
            "Connected" => Some(Self::Connected),
            "URLTemplate" => Some(Self::URLTemplate),
            "URLParameters" => Some(Self::URLParameters),
            "TOTPVersion" => Some(Self::TOTPVersion),
            "ChargingStationId" => Some(Self::ChargingStationId),
            "ValidityTime" => Some(Self::ValidityTime),
            "SharedSecret" => Some(Self::SharedSecret),
            "QRCodeQuality" => Some(Self::QRCodeQuality),
            _ => None,
        }
    }
    /// Parses any wire value, falling back to [`Self::Other`].
    ...
    pub fn data_type(&self) -> Option<&'static str> {
        match self {
            ...
            Self::DischargePower => Some("decimal"),
            ...
            Self::HandleFailedTariff => Some("OptionList"),
            ...
            Self::LocalFrequencyUpdateThreshold => Some("integer"),
            ...
            Self::SetpointPriority => Some("OptionList"),
            ...
            Self::SupportedAdditionalPurposes => Some("MemberList"),
            Self::SupportedEnergyTransferModes => Some("MemberList"),
            Self::SupportedFormats => Some("MemberList"),
            Self::SupportedIdTokenTypes => Some("MemberList"),
            Self::SupportedLimits => Some("MemberList"),
            Self::SupportedOperationModes => Some("MemberList"),
            Self::SupportedPriorities => Some("MemberList"),
            Self::SupportedProviders => Some("string"),
            Self::SupportedStates => Some("MemberList"),
            Self::SupportsDynamicProfiles => Some("boolean"),
            Self::SupportsEvseSleep => Some("boolean"),
            Self::SupportsExpiryDateTime => Some("boolean"),
            Self::SupportsLimitAtSoC => Some("boolean"),
            Self::SupportsMaxOfflineDuration => Some("boolean"),
            Self::SupportsRandomizedDelay => Some("boolean"),
            Self::SupportsUseLocalTime => Some("boolean"),
            Self::Suspending => Some("boolean"),
            Self::Suspension => Some("boolean"),
            Self::TargetSoc => Some("integer"),
            ...
            Self::UpstreamInterval => Some("integer"),
            Self::UpstreamMeasurands => Some("MemberList"),
            ...
            Self::WorkingMode => Some("OptionList"),
            ...
            Self::MaxW => None,
            Self::OverExcitedW => None,
            Self::OverExcitedPF => None,
            Self::UnderExcitedW => None,
            Self::UnderExcitedPF => None,
            Self::MaxVA => None,
            Self::MaxVar => None,
            Self::MaxVarNeg => None,
            Self::MaxChargeRateW => None,
            Self::MaxChargeRateVA => None,
            Self::VNom => None,
            Self::MaxV => None,
            Self::MinV => None,
            Self::ModesSupported => None,
            Self::InverterManufacturer => None,
            Self::InverterModel => None,
            Self::InverterSerialNumber => None,
            Self::InverterSwVersion => None,
            Self::InverterHwVersion => None,
            Self::IslandingDetectionMethod => None,
            Self::IslandingDetectionTripTime => None,
            Self::ReactiveSusceptance => None,
            Self::TargetSoC => None,
            ...
            Self::AuthorizeDirectPayment => None,
            Self::AuthorizationAmount => None,
            Self::IncrementalAuthorizationAmount => None,
            Self::IncrementalAuthorizationThreshold => None,
            Self::PaymentDetails => None,
            Self::SettlementByCSMS => None,
            Self::ReceiptServerUrl => None,
            Self::ReceiptByCSMS => None,
            Self::Merchant => None,
            Self::TerminalID => None,
            Self::PaymentServiceProvider => None,
            Self::Connected => None,
            Self::URLTemplate => None,
            Self::URLParameters => None,
            Self::TOTPVersion => None,
            Self::ChargingStationId => None,
            Self::ValidityTime => None,
            Self::SharedSecret => None,
            Self::QRCodeQuality => None,
            Self::Other(_) => None,
        }
    }
    ...
    pub fn unit(&self) -> Option<&'static str> {
        match self {
            ...
            Self::LocalFrequencyUpdateThreshold => Some("mHz"),
            ...
            Self::SupportedAdditionalPurposes => None,
            Self::SupportedEnergyTransferModes => None,
            Self::SupportedFormats => None,
            Self::SupportedIdTokenTypes => None,
            Self::SupportedLimits => None,
            Self::SupportedOperationModes => None,
            Self::SupportedPriorities => None,
            Self::SupportedProviders => None,
            Self::SupportedStates => None,
            Self::SupportsDynamicProfiles => None,
            Self::SupportsEvseSleep => None,
            Self::SupportsExpiryDateTime => None,
            Self::SupportsLimitAtSoC => None,
            Self::SupportsMaxOfflineDuration => None,
            Self::SupportsRandomizedDelay => None,
            Self::SupportsUseLocalTime => None,
            Self::Suspending => None,
            Self::Suspension => None,
            Self::TargetSoc => Some("Percent"),
            ...
            Self::UpstreamInterval => Some("s"),
            Self::UpstreamMeasurands => None,
...
impl ReasonCode {
    ...
    pub const ALL: &'static [Self] = &[
        ...
        Self::InvalidOperationMode,
        Self::NoFreqWattCurve,
        Self::NoPhaseForDC,
        Self::PhaseConflict,
        Self::NoSignalWattCurve,
        ...
        Self::BatteryUnknown,
        Self::BatteryType,
        Self::NoBatteryAvailable,
        Self::PriorityNetworkConf,
        Self::InvalidConfSlot,
    ...
    pub const fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::InvalidOperationMode => "InvalidOperationMode",
            Self::NoFreqWattCurve => "NoFreqWattCurve",
            Self::NoPhaseForDC => "NoPhaseForDC",
            Self::PhaseConflict => "PhaseConflict",
            Self::NoSignalWattCurve => "NoSignalWattCurve",
            ...
            Self::BatterySoHLow => "BatterySoHLow",
            Self::BatterySoC => "BatterySoC",
            Self::BatteryDamaged => "BatteryDamaged",
            Self::BatteryUnknown => "BatteryUnknown",
            Self::BatteryType => "BatteryType",
            Self::NoBatteryAvailable => "NoBatteryAvailable",
            Self::PriorityNetworkConf => "PriorityNetworkConf",
            Self::InvalidConfSlot => "InvalidConfSlot",
    ...
    pub fn from_wire(value: &str) -> Option<Self> {
        match value {
            ...
            "InvalidOperationMode" => Some(Self::InvalidOperationMode),
            "NoFreqWattCurve" => Some(Self::NoFreqWattCurve),
            "NoPhaseForDC" => Some(Self::NoPhaseForDC),
            "PhaseConflict" => Some(Self::PhaseConflict),
            "NoSignalWattCurve" => Some(Self::NoSignalWattCurve),
            ...
            "BatterySoHLow" => Some(Self::BatterySoHLow),
            "BatterySoC" => Some(Self::BatterySoC),
            "BatteryDamaged" => Some(Self::BatteryDamaged),
            "BatteryUnknown" => Some(Self::BatteryUnknown),
            "BatteryType" => Some(Self::BatteryType),
            "NoBatteryAvailable" => Some(Self::NoBatteryAvailable),
            "PriorityNetworkConf" => Some(Self::PriorityNetworkConf),
            "InvalidConfSlot" => Some(Self::InvalidConfSlot),
    ...
    pub const fn typically_used_for(&self) -> Option<&'static str> {
        match self {
            ...
            Self::InvalidOperationMode => Some("SetChargingProfile"),
            Self::NoFreqWattCurve => Some("SetChargingProfile"),
            Self::NoPhaseForDC => Some("SetChargingProfile"),
            Self::PhaseConflict => Some("SetChargingProfile"),
            Self::NoSignalWattCurve => Some("AFRRSignal"),
            ...
            Self::BatterySoHLow => Some("BatterySwap"),
            Self::BatterySoC => Some("BatterySwap"),
            Self::BatteryDamaged => Some("BatterySwap"),
            Self::BatteryUnknown => Some("BatterySwap"),
            Self::BatteryType => Some("BatterySwap"),
            Self::NoBatteryAvailable => Some("BatterySwap, RequestBatterySwap"),
            Self::PriorityNetworkConf => {
                Some("SetVariablesRequest of NetworkConfiguration")
            }
            Self::InvalidConfSlot => Some("SetNetworkProfileRequest"),
            Self::InvalidNetworkConf => {
                Some(
...
pub enum ConnectorType {
    /// Slot of a battery swap station to accept battery cartridges (type unspecified)
    #[cfg_attr(feature = "serde", serde(rename = "bBatterySlot"))]
    BBatterySlot,
    ...
    #[cfg_attr(feature = "serde", serde(rename = "cChaoJi"))]
    CChaoJi,
    ...
    #[cfg_attr(feature = "serde", serde(rename = "cGBT-DC"))]
    CGBTDC,
    /// Light Equipment Combined Charging System IS17017 (captive cabled)
    #[cfg_attr(feature = "serde", serde(rename = "cLECCS"))]
    CLECCS,
    /// Megawatt Charging System (captive cabled)
    #[cfg_attr(feature = "serde", serde(rename = "cMCS"))]
    CMCS,
    /// North American Charging Standard J3400 (captive cabled)
    #[cfg_attr(feature = "serde", serde(rename = "cNACS"))]
    CNACS,
    /// Built-in NACS to CCS1 adapter (captive cabled)
    #[cfg_attr(feature = "serde", serde(rename = "cNACS-CCS1"))]
    CNACSCCS1,
    /// Built-in CCS1 to NACS adapter (captive cabled)
    #[cfg_attr(feature = "serde", serde(rename = "cCCS1-NACS"))]
    CCCS1NACS,
    ...
    #[cfg_attr(feature = "serde", serde(rename = "cUltraChaoJi"))]
    CUltraChaoJi,
    ...
    #[cfg_attr(feature = "serde", serde(rename = "sType1"))]
    SType1,
...
impl ConnectorType {
    /// Every value this version's specification defines (33), in spec order.
    pub const ALL: &'static [Self] = &[
        Self::BBatterySlot,
        Self::CCCS1,
        Self::CCCS2,
    ...
    pub const fn as_str(&self) -> &'static str {
        match self {
            Self::BBatterySlot => "bBatterySlot",
            Self::CCCS1 => "cCCS1",
            Self::CCCS2 => "cCCS2",
            Self::CChaoJi => "cChaoJi",
            Self::CG105 => "cG105",
            Self::CGBTDC => "cGBT-DC",
            Self::CLECCS => "cLECCS",
            Self::CMCS => "cMCS",
            Self::CNACS => "cNACS",
            Self::CNACSCCS1 => "cNACS-CCS1",
            Self::CCCS1NACS => "cCCS1-NACS",
            Self::CTesla => "cTesla",
            Self::CType1 => "cType1",
            Self::CType2 => "cType2",
            Self::CUltraChaoJi => "cUltraChaoJi",
            Self::S3091P16A => "s309-1P-16A",
            Self::S3091P32A => "s309-1P-32A",
            Self::S3093P16A => "s309-3P-16A",
            Self::S3093P32A => "s309-3P-32A",
            Self::SBS1361 => "sBS1361",
            Self::SCEE77 => "sCEE-7-7",
            Self::SType1 => "sType1",
            Self::SType2 => "sType2",
            Self::SType3 => "sType3",
            Self::WInductive => "wInductive",
            Self::WResonant => "wResonant",
            Self::OppCharge => "OppCharge",
            Self::Other1PhMax16A => "Other1PhMax16A",
            Self::Other1PhOver16A => "Other1PhOver16A",
            Self::Other3Ph => "Other3Ph",
            Self::Pan => "Pan",
    ...
    pub fn from_wire(value: &str) -> Option<Self> {
        match value {
            "bBatterySlot" => Some(Self::BBatterySlot),
            "cCCS1" => Some(Self::CCCS1),
            "cCCS2" => Some(Self::CCCS2),
            "cChaoJi" => Some(Self::CChaoJi),
            "cG105" => Some(Self::CG105),
            "cGBT-DC" => Some(Self::CGBTDC),
            "cLECCS" => Some(Self::CLECCS),
            "cMCS" => Some(Self::CMCS),
            "cNACS" => Some(Self::CNACS),
            "cNACS-CCS1" => Some(Self::CNACSCCS1),
            "cCCS1-NACS" => Some(Self::CCCS1NACS),
            "cTesla" => Some(Self::CTesla),
            "cType1" => Some(Self::CType1),
            "cType2" => Some(Self::CType2),
            "cUltraChaoJi" => Some(Self::CUltraChaoJi),
            "s309-1P-16A" => Some(Self::S3091P16A),
            "s309-1P-32A" => Some(Self::S3091P32A),
            "s309-3P-16A" => Some(Self::S3093P16A),
            "s309-3P-32A" => Some(Self::S3093P32A),
            "sBS1361" => Some(Self::SBS1361),
            "sCEE-7-7" => Some(Self::SCEE77),
            "sType1" => Some(Self::SType1),
            "sType2" => Some(Self::SType2),
            "sType3" => Some(Self::SType3),
            "wInductive" => Some(Self::WInductive),
            "wResonant" => Some(Self::WResonant),
            "OppCharge" => Some(Self::OppCharge),
            "Other1PhMax16A" => Some(Self::Other1PhMax16A),
            "Other1PhOver16A" => Some(Self::Other1PhOver16A),
            "Other3Ph" => Some(Self::Other3Ph),
            "Pan" => Some(Self::Pan),
...
impl core::fmt::Display for ConnectorType {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.as_str())
    }
}
impl core::str::FromStr for ConnectorType {
    type Err = UnknownValue;
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        Self::from_wire(value).ok_or(UnknownValue)
    }
...
impl IdTokenType {
    /// Every value this version's specification defines (11), in spec order.
    pub const ALL: &'static [Self] = &[
        Self::Central,
        Self::DirectPayment,
        Self::EMAID,
    ...
    pub const fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::NoAuthorization => "NoAuthorization",
            Self::VIN => "VIN",
        }
    }
    /// Parses a wire value, returning `None` for values the
    ...
    pub fn from_wire(value: &str) -> Option<Self> {
        match value {
            ...
            "DirectPayment" => Some(Self::DirectPayment),
            "eMAID" => Some(Self::EMAID),
            "EVCCID" => Some(Self::EVCCID),
            "ISO14443" => Some(Self::ISO14443),
            "ISO15693" => Some(Self::ISO15693),
            "KeyCode" => Some(Self::KeyCode),
            "Local" => Some(Self::Local),
            "MacAddress" => Some(Self::MacAddress),
            "NoAuthorization" => Some(Self::NoAuthorization),
            "VIN" => Some(Self::VIN),
            _ => None,
        }
    }
}
impl core::fmt::Display for IdTokenType {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.as_str())
    }
}
impl core::str::FromStr for IdTokenType {
    type Err = UnknownValue;
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        Self::from_wire(value).ok_or(UnknownValue)
    }
...
impl ChargingLimitSource {
    /// Every value this version's specification defines (4), in spec order.
    pub const ALL: &'static [Self] = &[Self::EMS, Self::Other, Self::SO, Self::CSO];
    /// This value as it appears on the wire.
    pub const fn as_str(&self) -> &'static str {
        match self {
            Self::EMS => "EMS",
...
impl core::fmt::Display for ChargingLimitSource {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.as_str())
    }
}
impl core::str::FromStr for ChargingLimitSource {
    type Err = UnknownValue;
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        Self::from_wire(value).ok_or(UnknownValue)
    }
...
impl PaymentBrand {
    /// Every value this version's specification defines (22), in spec order.
    pub const ALL: &'static [Self] = &[
        Self::AMEX,
        Self::ApplePay,
        Self::Bancontact,
    ...
    pub const fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::BankAxept => "BankAxept",
            Self::Carnet => "Carnet",
            Self::CartesBancaires => "CartesBancaires",
            Self::Dankort => "Dankort",
            Self::Diners => "Diners",
            Self::Discover => "Discover",
            Self::EftposAustralia => "EftposAustralia",
            Self::Elo => "Elo",
    ...
    pub fn from_wire(value: &str) -> Option<Self> {
        match value {
            "AMEX" => Some(Self::AMEX),
            "ApplePay" => Some(Self::ApplePay),
            "Bancontact" => Some(Self::Bancontact),
            "BankAxept" => Some(Self::BankAxept),
            "Carnet" => Some(Self::Carnet),
            "CartesBancaires" => Some(Self::CartesBancaires),
            "Dankort" => Some(Self::Dankort),
            "Diners" => Some(Self::Diners),
            "Discover" => Some(Self::Discover),
            "EftposAustralia" => Some(Self::EftposAustralia),
            "Elo" => Some(Self::Elo),
            "Girocard" => Some(Self::Girocard),
            ...
            "SamsungPay" => Some(Self::SamsungPay),
...
impl core::fmt::Display for PaymentBrand {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.as_str())
    }
}
impl core::str::FromStr for PaymentBrand {
    type Err = UnknownValue;
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        Self::from_wire(value).ok_or(UnknownValue)
    }
...
impl PaymentRecognition {
    /// Every value this version's specification defines (9), in spec order.
    pub const ALL: &'static [Self] = &[
        Self::CC,
        Self::Debit,
        Self::Alipay,
...
impl core::fmt::Display for PaymentRecognition {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.as_str())
    }
}
impl core::str::FromStr for PaymentRecognition {
    type Err = UnknownValue;
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        Self::from_wire(value).ok_or(UnknownValue)
    }
...
pub enum SigningMethod {
    #[cfg_attr(feature = "serde", serde(rename = "ECDSA-secp192k1-SHA256"))]
    ECDSAsecp192k1SHA256,
    #[cfg_attr(feature = "serde", serde(rename = "ECDSA-secp256k1-SHA256"))]
    ECDSAsecp256k1SHA256,
    #[cfg_attr(feature = "serde", serde(rename = "ECDSA-secp192r1-SHA256"))]
    ECDSAsecp192r1SHA256,
    #[cfg_attr(feature = "serde", serde(rename = "ECDSA-secp256r1-SHA256"))]
    ECDSAsecp256r1SHA256,
    #[cfg_attr(feature = "serde", serde(rename = "ECDSA-brainpool256r1-SHA256"))]
    ECDSAbrainpool256r1SHA256,
    #[cfg_attr(feature = "serde", serde(rename = "ECDSA-secp384r1-SHA256"))]
    ECDSAsecp384r1SHA256,
    #[cfg_attr(feature = "serde", serde(rename = "ECDSA-brainpool384r1-SHA256"))]
    ECDSAbrainpool384r1SHA256,
}
impl SigningMethod {
    /// Every value this version's specification defines (7), in spec order.
    pub const ALL: &'static [Self] = &[
        Self::ECDSAsecp192k1SHA256,
    ...
    pub fn from_wire(value: &str) -> Option<Self> {
        match value {
            "ECDSA-secp192k1-SHA256" => Some(Self::ECDSAsecp192k1SHA256),
            "ECDSA-secp256k1-SHA256" => Some(Self::ECDSAsecp256k1SHA256),
            "ECDSA-secp192r1-SHA256" => Some(Self::ECDSAsecp192r1SHA256),
            "ECDSA-secp256r1-SHA256" => Some(Self::ECDSAsecp256r1SHA256),
            "ECDSA-brainpool256r1-SHA256" => Some(Self::ECDSAbrainpool256r1SHA256),
            "ECDSA-secp384r1-SHA256" => Some(Self::ECDSAsecp384r1SHA256),
            "ECDSA-brainpool384r1-SHA256" => Some(Self::ECDSAbrainpool384r1SHA256),
            _ => None,
        }
    }
    /// The signature algorithm.
    pub const fn algorithm(&self) -> Option<&'static str> {
        match self {
            Self::ECDSAsecp192k1SHA256 => Some("ECDSA"),
            Self::ECDSAsecp256k1SHA256 => Some("ECDSA"),
            Self::ECDSAsecp192r1SHA256 => Some("ECDSA"),
            Self::ECDSAsecp256r1SHA256 => Some("ECDSA"),
            Self::ECDSAbrainpool256r1SHA256 => Some("ECDSA"),
            Self::ECDSAsecp384r1SHA256 => Some("ECDSA"),
            Self::ECDSAbrainpool384r1SHA256 => Some("ECDSA"),
        }
    }
    /// The elliptic curve.
    pub const fn curve(&self) -> Option<&'static str> {
        match self {
            Self::ECDSAsecp192k1SHA256 => Some("secp192k1"),
            Self::ECDSAsecp256k1SHA256 => Some("secp256k1"),
            Self::ECDSAsecp192r1SHA256 => Some("secp192r1"),
            Self::ECDSAsecp256r1SHA256 => Some("secp256r1"),
            Self::ECDSAbrainpool256r1SHA256 => Some("brainpool256r1"),
            Self::ECDSAsecp384r1SHA256 => Some("secp384r1"),
            Self::ECDSAbrainpool384r1SHA256 => Some("brainpool384r1"),
        }
    }
    /// The key length, as the spec states it.
    pub const fn key_length(&self) -> Option<&'static str> {
        match self {
            Self::ECDSAsecp192k1SHA256 => Some("192 bits"),
            Self::ECDSAsecp256k1SHA256 => Some("256 bits"),
            Self::ECDSAsecp192r1SHA256 => Some("192 bits"),
            Self::ECDSAsecp256r1SHA256 => Some("256 bits"),
            Self::ECDSAbrainpool256r1SHA256 => Some("256 bits"),
            Self::ECDSAsecp384r1SHA256 => Some("384 bits"),
            Self::ECDSAbrainpool384r1SHA256 => Some("384 bits"),
        }
    }
    /// The hash algorithm paired with the signature.
    pub const fn hash_algorithm(&self) -> Option<&'static str> {
        match self {
            Self::ECDSAsecp192k1SHA256 => Some("SHA-256"),
            Self::ECDSAsecp256k1SHA256 => Some("SHA-256"),
            Self::ECDSAsecp192r1SHA256 => Some("SHA-256"),
            Self::ECDSAsecp256r1SHA256 => Some("SHA-256"),
            Self::ECDSAbrainpool256r1SHA256 => Some("SHA-256"),
            Self::ECDSAsecp384r1SHA256 => Some("SHA-256"),
            Self::ECDSAbrainpool384r1SHA256 => Some("SHA-256"),
        }
    }
}
impl core::fmt::Display for SigningMethod {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.as_str())
    }
}
impl core::str::FromStr for SigningMethod {
    type Err = UnknownValue;
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        Self::from_wire(value).ok_or(UnknownValue)
    }
...
pub enum AdditionalInfoType {
    ...
    ISO14443SubType,
    /// Payment Service Provider reference id for payment session. Only use when PspRef is not in _idToken_.
    PspRef,
...
impl AdditionalInfoType {
    /// Every value this version's specification defines (13), in spec order.
    pub const ALL: &'static [Self] = &[
        Self::EVCCID,
        Self::ISO14443SubType,
        Self::PspRef,
        Self::SessionRef,
        Self::MerchantRef,
        Self::PaymentBrand,
        Self::ReadingMethod,
        Self::PaymentRecognition,
        Self::CardBin,
        Self::CardLast4Digits,
        Self::CardExpiryDate,
        Self::HashedCardNr,
        Self::WalletUserId,
    ];
    ...
    pub fn from_wire(value: &str) -> Option<Self> {
        match value {
            "EVCCID" => Some(Self::EVCCID),
            "ISO14443SubType" => Some(Self::ISO14443SubType),
            "PspRef" => Some(Self::PspRef),
            "SessionRef" => Some(Self::SessionRef),
            "MerchantRef" => Some(Self::MerchantRef),
            "PaymentBrand" => Some(Self::PaymentBrand),
            "ReadingMethod" => Some(Self::ReadingMethod),
            "PaymentRecognition" => Some(Self::PaymentRecognition),
            "CardBin" => Some(Self::CardBin),
            "CardLast4Digits" => Some(Self::CardLast4Digits),
            "CardExpiryDate" => Some(Self::CardExpiryDate),
            "HashedCardNr" => Some(Self::HashedCardNr),
            "WalletUserId" => Some(Self::WalletUserId),
            _ => None,
        }
    }
}
impl core::fmt::Display for AdditionalInfoType {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str(self.as_str())
    }
}
impl core::str::FromStr for AdditionalInfoType {
    type Err = UnknownValue;
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        Self::from_wire(value).ok_or(UnknownValue)
    }
...
pub const DEVICE_MODEL: &[DeviceModelEntry] = &[
    ...
    DeviceModelEntry {
        component: Some("V2XChargingCtrlr"),
        variable: "Enabled",
        attributes: None,
        instance: None,
        required: Some("yes"),
    ...
    DeviceModelEntry {
        component: Some("BatterySwapCtrlr"),
        variable: "TargetSoC",
        attributes: None,
        instance: None,
        required: Some("no"),
    ...
    DeviceModelEntry {
        component: Some("BatteryCartridge"),
        variable: "SoC",
        attributes: None,
        instance: None,
        required: Some("no"),
    ...
    DeviceModelEntry {
        component: Some("WebPaymentsCtrlr"),
