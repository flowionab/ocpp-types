//! RPC framework error codes from Table 8 ("Valid Error Codes") of the
//! OCPP-J 2.0.1 specification -- the `errorCode` values a `CALLERROR`
//! frame may carry.

/// One of the error codes OCPP-J 2.0.1 allows in a `CALLERROR` frame.
///
/// Two codes changed from 1.6J: `FormationViolation` was renamed to
/// `FormatViolation`, and `OccurenceConstraintViolation`'s missing "r" was
/// fixed to `OccurrenceConstraintViolation`. Two codes were added:
/// `MessageTypeNotSupported` and `RpcFrameworkError`.
...
mod tests {
    ...
    fn serializes_using_the_exact_wire_spelling() {
        // 2.0.1 corrected 1.6J's "OccurenceConstraintViolation" typo and
        // renamed "FormationViolation" to "FormatViolation" -- this locks
        // in that those aren't accidentally reused from v16.
        let mut buf = [0u8; 64];
        let json =
